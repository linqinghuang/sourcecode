...
//	uchar datedisplay[7];
 ...
 //lcd_init2();	
	...
	{	LED=0;}	//检查nrf是否存在
	LED=1;
	RX_Mode();//	 其后接一个界面显示函数然后是while（！（NRF24L01_RxPacket(RxBuf)==0））；
interface();
	...
	while(1)
	{
		...
		{
			for(i=0;i<8;i++)
			datedisplay[i]=RxBuf[i];	//将字符串转换为整形，便于显示	如果使用的是液晶显示，此处不需转换
		}
	//这里加checkbutton
	checkbutton();
//	lcd12864(datedisplay);
	lcddisplay(datedisplay);
	}		
}